...
class BSite(XMLStorage):
    ...
    def __init__(self, bindingsite, pdbid):
        ...
        self.interaction_positions = self.get_interaction_positions()
    ...
    def get_interaction_positions(self):
        """get all residue positions of any interaction"""

        positions = {'hydrophobics': [(b.restype, b.resnr) for b in self.hydrophobics],
                     'hbonds': [(b.restype, b.resnr) for b in self.hbonds],
                     'wbridges': [(b.restype, b.resnr) for b in self.wbridges],
                     'sbridges': [(b.restype, b.resnr) for b in self.sbridges],
                     'pistacks': [(b.restype, b.resnr) for b in self.pi_stacks],
                     'pications': [(b.restype, b.resnr) for b in self.pi_cations],
                     'halogens': [(b.restype, b.resnr) for b in self.halogens],
                     'metal': [(b.restype, b.resnr) for b in self.metal_complexes]}

        positions['all'] =positions['hydrophobics'] + positions['hbonds'] + positions['wbridges'] + \
                          positions['sbridges'] + positions['pistacks'] + positions['pications'] + positions['halogens'] + positions[
                              'metal']
...
# Fingerprints based on PLIP output
import numpy as np

class Fingerprint:
    """Provides the basic functions for fingerprints"""

    def __init__(self,bsite):
        self.id = bsite.uniqueid

    def get_bit_fp(self):
        """one bit per interaction and residue - no info on how often interaction-residue pair appears"""
        return list(map(lambda x: 0 if x == 0 else 1,self.fp))

    def get_fp(self):
        """number of appearances for each interaction-residue pair"""
        return self.fp

class SimpleFP(Fingerprint):
    """
    the simplest fingerprint simply counts the number of interactions for each type

    possible adaption: separate for T and P pi-stacking by adapting get_counts()
    """

    def __init__(self, bsite):
        super().__init__(bsite)
        self.fp = np.array([
            bsite.counts['hydrophobics'],
            bsite.counts['hbond_back'],
            bsite.counts['hbond_nonback'],
            bsite.counts['wbridges'],
            bsite.counts['sbridges'],
            bsite.counts['pistacks'],
            bsite.counts['pications'],
            bsite.counts['halogens'],
            bsite.counts['metal']
        ])


class ResidueInteractionFP(Fingerprint):
    """ A target focused fingerprint taking into account interaction type and interacting target residue"""

    def __init__(self, bsite):
        super().__init__(bsite)
        aa = ['ALA', 'ARG', 'ASN', 'ASP', 'CYS', 'GLN', 'GLU', 'GLY', 'HIS', 'ILE', 'LEU', 'LYS', 'MET', 'PHE', 'PRO', 'SER', 'THR', 'TRP', 'TYR', 'VAL', 'XAA']
        dna = ['DT', 'DA', 'DC', 'DG']
        rna = ['U', 'A', 'C', 'G']
        fp = {k: [0]*8 for k in aa + dna + rna + ['Unknown']}

        if bsite.has_interactions:
            interactions = [bsite.hydrophobics] + [bsite.hbonds] + [bsite.wbridges] + [bsite.sbridges] + [bsite.pi_stacks] + [bsite.pi_cations] + [bsite.halogens] + [bsite.metal_complexes]
            for i in range(8):
                for ia in interactions[i]:
                    try:
                        fp[ia.restype][i] += 1
                    except KeyError:
                        fp['Unknown'][i] += 1
                        print(f"Unknown residue type '{ia.restype}'")

        self.fp = np.array([x for sublist in fp.values() for x in sublist])

def tanimoto(fp1, fp2):
    c = sum([min(x,y) for (x,y) in zip(fp1,fp2)])
    a = sum(fp1)
    b = sum(fp2)
    return c/(a+b-c)